...
Locator: getByText('Questionnaire_1778837628798 Link').getByRole('checkbox', { name: 'ql._id' })
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByText('Questionnaire_1778837628798 Link').getByRole('checkbox', { name: 'ql._id' })
      ...
      - generic [ref=e243]:
        - generic [ref=e245]:
          - img [ref=e247]:
            - img
          - generic [ref=e248]:
            - generic [ref=e250]: "Risk: Risk_1778837628798"
      ...
      - generic [ref=e663]:
        ...
        - button "select country QA Automation" [ref=e664]:
          - generic [ref=e665]:
            ...
            - generic [ref=e667]: ▼
      - generic [ref=e668]:
        - textbox "Enter search" [ref=e671]: Questionnaire_1778837628798
        - generic [ref=e673]: search
    - progressbar [ref=e674]
    - generic [ref=e680]:
      - button "Cancel" [ref=e681] [cursor=pointer]
      - button "OK" [disabled] [ref=e682]
```
...
> 187 |     await expect(checkbox).toBeVisible({ timeout: 15000 });
  ...
  228 |   }
  229 | 
  230 |   async clickOnQuestionnaire(questionnaireName: string) {
  231 |     const questionnaire = this.page.locator(`[data-testid="${questionnaireName}"]`);
  232 |     await expect(questionnaire).toBeVisible({ timeout: 10000 });
  233 |     await this.actions.click(questionnaire);
  234 |   }
  235 | 
  236 |   async openRiskByName(riskName: string) {
  237 |     const riskCell = this.page.getByRole('gridcell', { name: riskName });
  238 | 
  239 |     await this.actions.click(riskCell);
  240 |   }
  241 | 
  242 |   async verifyQuestionnaireIsFinished(questionnaireName: string) {
  243 |     const finishedIcon = this.page.locator(`[data-testid="${questionnaireName}"] [alt="finished"]`);
  244 | 
  245 |     await this.actions.waitForVisible(finishedIcon);
  246 | 
  247 |     const progress = this.page.locator(`[data-testid="${questionnaireName}"] div[progressvalue]`);
  248 | 
  249 |     await expect(progress).toContainText('100%');
  250 |   }
  251 | 
  252 |   async searchRisk(riskName: string) {
  253 |     await this.actions.click(this.searchIcon);
  254 |     await this.actions.type(this.searchInput, riskName);
  255 |     await this.searchInput.press('Enter');
  256 |   }
  257 | 
  258 |   async clickCloseDialogButton() {
  259 |     if (await this.closeDialogButton.isVisible()) {
  260 |       await this.actions.click(this.closeDialogButton);
  261 |     }
  262 |     else {
  263 |       const closeDialogueNew = this.page.getByRole('button', { name: 'Close dialog' });
  264 |       await this.actions.click(closeDialogueNew);
  265 |     }
  266 |   }
  267 | 
  268 |   async clickCloseQuestionnaireButton() {
  269 |     const closeQuestionnaireButton = this.page.locator('[data-testid="moqnr-close-icon"]');
  270 |     await this.actions.click(closeQuestionnaireButton);
  271 |   }
  272 | 
  273 |   async verifyAuditLogFieldValue(fieldName: string, expectedValue: string) {
  274 |     await this.actions.click(this.latestHistoryLog);
  275 | 
  276 |     const field = this.page.getByRole('textbox', { name: fieldName });
  277 |     await expect(field).toHaveValue(expectedValue);
  278 | 
  279 |     await this.clickCloseDialogButton();
  280 |   }
  281 | 
  282 |   async clickOnVariableIcon() {
  283 |     await this.actions.click(this.variableIcon);
  284 |   }
  285 | 
  286 |   async clickOnMoreOptionsInCard(cardName: string) {
  287 |     const moreOption = this.page
```