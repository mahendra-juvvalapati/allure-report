...
Call log:
  ...
  - waiting for locator('.row:has-text("Set Versionreminderdate4 to")').locator('input[type="number"]')
    33 × locator resolved to <input tabindex="0" type="number" autocomplete="off" can-trim-on-change-done="true" class="q-field__native q-placeholder" id="f_ac1b0ff6-9c90-447d-a6f7-187f6d5c0a06"/>
       - unexpected value ""

```

```yaml
- spinbutton "Period Number"
```
