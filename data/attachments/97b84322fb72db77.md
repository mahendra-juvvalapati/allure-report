# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: E2E_Tests/FC021_questionnaire.spec.ts >> FC_021_Questionnaire Flow >> Configure questionnaire with 'Add documents as evidence' and verify 'Show' action is executed when Question condition is triggered on 'Completing the page' and check Questionnaire 'Set Value to Variable' action
- Location: tests/E2E_Tests/FC021_questionnaire.spec.ts:31:9

# Error details

```
TimeoutError: locator.click: Timeout 15000ms exceeded.
Call log:
  - waiting for getByRole('button', { name: 'clear' })
    - locator resolved to <button type="button" ng-transclude="" aria-label="clear" ng-if="manageQuestionnaire" ng-click="manageableobject.manageMOQuestionnaire()" class="md-icon-button md-button ng-scope md-ink-ripple">…</button>
  - attempting click action
    2 × waiting for element to be visible, enabled and stable
      - element is visible, enabled and stable
      - scrolling into view if needed
      - done scrolling
      - <div tabindex="-1" class="md-dialog-container ng-scope">…</div> intercepts pointer events
    - retrying click action
    - waiting 20ms
    2 × waiting for element to be visible, enabled and stable
      - element is visible, enabled and stable
      - scrolling into view if needed
      - done scrolling
      - <div tabindex="-1" class="md-dialog-container ng-scope">…</div> intercepts pointer events
    - retrying click action
      - waiting 100ms
    21 × waiting for element to be visible, enabled and stable
       - element is visible, enabled and stable
       - scrolling into view if needed
       - done scrolling
       - <div tabindex="-1" class="md-dialog-container ng-scope">…</div> intercepts pointer events
     - retrying click action
       - waiting 500ms

```

# Page snapshot

```yaml
- generic [ref=e1]:
  - text: ×
  - link [ref=e3] [cursor=pointer]:
    - /url: javascript:void(0)
    - img [ref=e4]
  - main [ref=e7]:
    - generic [ref=e10]:
      - generic [ref=e11]:
        - generic [ref=e13]:
          - button [ref=e14] [cursor=pointer]:
            - img [ref=e15]
          - generic [ref=e16]:
            - img [ref=e20]
            - heading [level=2] [ref=e24]: QualiTlabs Hyd
          - generic [ref=e28] [cursor=pointer]: search
          - button [ref=e29] [cursor=pointer]:
            - img [ref=e30]:
              - img
          - button [ref=e32] [cursor=pointer]:
            - img [ref=e34]:
              - img [ref=e35]
          - generic [ref=e40]:
            - button [ref=e41] [cursor=pointer]:
              - img [ref=e43]:
                - img [ref=e44]
            - status [ref=e45]: "50"
          - button [ref=e46] [cursor=pointer]:
            - generic [ref=e47]: fullscreen
          - button [ref=e48] [cursor=pointer]:
            - img [ref=e49]:
              - img
            - generic [ref=e50]: "1"
          - button [ref=e51] [cursor=pointer]:
            - generic [ref=e52]: info
        - generic [ref=e53]:
          - generic [ref=e54]:
            - img [ref=e55] [cursor=pointer]
            - button [ref=e56] [cursor=pointer]:
              - generic [ref=e57]: dehaze
          - list [ref=e58]:
            - generic [ref=e59]:
              - button [ref=e60]
              - generic [ref=e61]: mahendra j
            - button [ref=e62] [cursor=pointer]:
              - generic [ref=e63]: keyboard_arrow_down
              - generic [ref=e64]: mahendra.juvvalapati@qualitlabs.com
            - list [ref=e65]:
              - button [ref=e66] [cursor=pointer]:
                - generic [ref=e67]: account_box
                - generic [ref=e68]: My Data
              - button [ref=e69] [cursor=pointer]:
                - generic [ref=e70]: settings
                - generic [ref=e71]: Settings
              - button [ref=e72] [cursor=pointer]:
                - generic [ref=e73]: exit_to_app
                - generic [ref=e74]: Logout
          - list [ref=e76]:
            - button [ref=e77] [cursor=pointer]:
              - img [ref=e78]:
                - img
              - generic [ref=e79]: Add new user
            - button [ref=e80] [cursor=pointer]:
              - generic [ref=e81]: business
              - generic [ref=e82]: Environments
            - button [ref=e83] [cursor=pointer]:
              - generic [ref=e84]: home
              - generic [ref=e85]: Dashboard
            - button [ref=e86] [cursor=pointer]:
              - generic [ref=e87]: widgets
              - generic [ref=e88]: Modules
              - generic [ref=e89]: keyboard_arrow_down
            - list [ref=e90]:
              - button [ref=e91] [cursor=pointer]:
                - img [ref=e92]:
                  - img
                - generic [ref=e93]: Strategy
              - button [ref=e94] [cursor=pointer]:
                - img [ref=e95]:
                  - img
                - generic [ref=e96]: Policy
              - button [ref=e97] [cursor=pointer]:
                - img [ref=e98]:
                  - img
                - generic [ref=e99]: Programs
              - button [ref=e100] [cursor=pointer]:
                - img [ref=e101]:
                  - img
                - generic [ref=e102]: projects
              - button [ref=e103] [cursor=pointer]:
                - img [ref=e104]:
                  - img
                - generic [ref=e105]: Dialogues
              - button [ref=e106] [cursor=pointer]:
                - img [ref=e107]:
                  - img
                - generic [ref=e108]: Objectives
              - button [ref=e109] [cursor=pointer]:
                - img [ref=e110]:
                  - img
                - generic [ref=e111]: Processes
              - button [ref=e112] [cursor=pointer]:
                - img [ref=e113]:
                  - img
                - generic [ref=e114]: Organisation functions
              - button [ref=e115] [cursor=pointer]:
                - img [ref=e116]:
                  - img
                - generic [ref=e117]: KSF's - Opportunities
              - button [ref=e118] [cursor=pointer]:
                - img [ref=e119]:
                  - img
                - generic [ref=e120]: Stakeholders
              - button [ref=e121] [cursor=pointer]:
                - img [ref=e122]:
                  - img
                - generic [ref=e123]: Risks
              - button [ref=e124] [cursor=pointer]:
                - img [ref=e125]:
                  - img
                - generic [ref=e126]: Opportunities
              - button [ref=e127] [cursor=pointer]:
                - img [ref=e128]:
                  - img
                - generic [ref=e129]: Standards Frameworks
              - button [ref=e130] [cursor=pointer]:
                - img [ref=e131]:
                  - img
                - generic [ref=e132]: Laws and regulations
              - button [ref=e133] [cursor=pointer]:
                - img [ref=e134]:
                  - img
                - generic [ref=e135]: Measures
              - button [ref=e136] [cursor=pointer]:
                - img [ref=e137]:
                  - img
                - generic [ref=e138]: Assets
              - button [ref=e139] [cursor=pointer]:
                - img [ref=e140]:
                  - img
                - generic [ref=e141]: Information systems
              - button [ref=e142] [cursor=pointer]:
                - img [ref=e143]:
                  - img
                - generic [ref=e144]: Processing
              - button [ref=e145] [cursor=pointer]:
                - img [ref=e146]:
                  - img
                - generic [ref=e147]: Audits
              - button [ref=e148] [cursor=pointer]:
                - img [ref=e149]:
                  - img
                - generic [ref=e150]: Impact Assessments
              - button [ref=e151] [cursor=pointer]:
                - img [ref=e152]:
                  - img
                - generic [ref=e153]: Audit sets
              - button [ref=e154] [cursor=pointer]:
                - img [ref=e155]:
                  - img
                - generic [ref=e156]: Tests
              - button [ref=e157] [cursor=pointer]:
                - img [ref=e158]:
                  - img
                - generic [ref=e159]: Findings
              - button [ref=e160] [cursor=pointer]:
                - img [ref=e161]:
                  - img
                - generic [ref=e162]: Problems
              - button [ref=e163] [cursor=pointer]:
                - img [ref=e164]:
                  - img
                - generic [ref=e165]: Changes
              - button [ref=e166] [cursor=pointer]:
                - img [ref=e167]:
                  - img
                - generic [ref=e168]: Cases
              - button [ref=e169] [cursor=pointer]:
                - img [ref=e170]:
                  - img
                - generic [ref=e171]: Tasks
              - button [ref=e172] [cursor=pointer]:
                - img [ref=e173]:
                  - img
                - generic [ref=e174]: Documents
              - button [ref=e175] [cursor=pointer]:
                - img [ref=e176]:
                  - img
                - generic [ref=e177]: News items
              - button [ref=e178] [cursor=pointer]:
                - img [ref=e179]:
                  - img
                - generic [ref=e180]: Related parties
              - button [ref=e181] [cursor=pointer]:
                - img [ref=e182]:
                  - img
                - generic [ref=e183]: Contracts
              - button [ref=e184] [cursor=pointer]:
                - img [ref=e185]:
                  - img
                - generic [ref=e186]: Vendors
              - button [ref=e187] [cursor=pointer]:
                - img [ref=e188]:
                  - img
                - generic [ref=e189]: Arrangements
              - button [ref=e190] [cursor=pointer]:
                - img [ref=e191]:
                  - img
                - generic [ref=e192]: Insurances
              - button [ref=e193] [cursor=pointer]:
                - img [ref=e194]:
                  - img
                - generic [ref=e195]: Key Figures
              - button [ref=e196] [cursor=pointer]:
                - img [ref=e197]:
                  - img
                - generic [ref=e198]: Settings
              - button [ref=e199] [cursor=pointer]:
                - img [ref=e200]:
                  - img
                - generic [ref=e201]: BCM Activities
              - button [ref=e202] [cursor=pointer]:
                - img [ref=e203]:
                  - img
                - generic [ref=e204]: BCM Plans
              - button [ref=e205] [cursor=pointer]:
                - img [ref=e206]:
                  - img
                - generic [ref=e207]: Context analysis
              - button [ref=e208] [cursor=pointer]:
                - img [ref=e209]:
                  - img
                - generic [ref=e210]: Situations
          - generic [ref=e213]:
            - button [ref=e214] [cursor=pointer]:
              - generic [ref=e216]:
                - img [ref=e218]
                - generic [ref=e220]: close
            - menu:
              - generic:
                - generic:
                  - generic:
                    - img
      - generic [ref=e226]:
        - generic [ref=e228]:
          - generic [ref=e229]:
            - generic [ref=e231]:
              - generic [ref=e232] [cursor=pointer]:
                - generic: apps
                - generic:
                  - img
              - list [ref=e233]:
                - listitem [ref=e234]:
                  - generic [ref=e236]:
                    - generic:
                      - img
                    - generic [ref=e237]: Risk
            - button [ref=e240] [cursor=pointer]:
              - generic [ref=e241]: version:No repeatplan - 2 Version name
              - generic [ref=e242]: keyboard_arrow_down
            - generic [ref=e243]:
              - button [ref=e244] [cursor=pointer]:
                - img [ref=e245]:
                  - generic [ref=e246]: featured_play_list
              - button [ref=e247] [cursor=pointer]:
                - img [ref=e248]:
                  - generic [ref=e249]: clear
            - generic [ref=e250]:
              - button [ref=e252] [cursor=pointer]:
                - img [ref=e253]:
                  - img
              - button [ref=e254] [cursor=pointer]:
                - img [ref=e256]:
                  - img [ref=e257]
          - text: ▼
        - generic [ref=e258]:
          - generic [ref=e260]:
            - img [ref=e262]:
              - img
            - generic [ref=e263]:
              - generic [ref=e265]: "Risk: Risk_1780566166653"
              - button [ref=e267] [cursor=pointer]:
                - img [ref=e268]: file_copy
              - button [ref=e269] [cursor=pointer]:
                - img [ref=e270]:
                  - img
              - generic [ref=e271]:
                - button [disabled] [ref=e272]:
                  - img [ref=e273]:
                    - img
                - button [disabled] [ref=e274]:
                  - img [ref=e275]:
                    - img
          - generic [ref=e276]:
            - img:
              - img
            - generic [ref=e277]:
              - generic [ref=e278]:
                - button [disabled] [ref=e280]:
                  - img [ref=e281]:
                    - img
                - button [ref=e282] [cursor=pointer]:
                  - img [ref=e283]:
                    - img
                - generic [ref=e284]:
                  - tablist [ref=e285]:
                    - tab [ref=e286] [cursor=pointer]:
                      - generic [ref=e288]: Gross risk
                    - tab [ref=e289] [cursor=pointer]:
                      - generic [ref=e291]: Net risk
                    - tab [ref=e292] [cursor=pointer]:
                      - generic [ref=e294]: Bowtie
                    - tab [ref=e295] [cursor=pointer]:
                      - generic [ref=e297]: Measures
                    - tab [ref=e298] [cursor=pointer]:
                      - generic [ref=e300]: Kritieke Bedrijfsfuncties
                    - tab [selected] [ref=e301]:
                      - generic [ref=e303]: Dialogue
                    - tab [ref=e304] [cursor=pointer]:
                      - generic [ref=e306]: Chat
                    - tab [ref=e307] [cursor=pointer]:
                      - generic [ref=e309]: Information systems
                    - tab [ref=e310] [cursor=pointer]:
                      - generic [ref=e312]: Related
                    - tab [ref=e313] [cursor=pointer]:
                      - generic [ref=e315]: Tasks
                    - tab [ref=e316] [cursor=pointer]:
                      - generic [ref=e318]: Key Indicators
                    - tab [ref=e319] [cursor=pointer]:
                      - generic [ref=e321]: History
                    - tab [ref=e322] [cursor=pointer]:
                      - generic [ref=e324]: mo data
                    - tab [ref=e325] [cursor=pointer]:
                      - generic [ref=e327]: Risks
                  - generic [ref=e329]:
                    - generic [ref=e332] [cursor=pointer]: Gross risk
                    - generic [ref=e335] [cursor=pointer]: Net risk
                    - generic [ref=e338] [cursor=pointer]: Bowtie
                    - generic [ref=e341] [cursor=pointer]: Measures
                    - generic [ref=e344] [cursor=pointer]: Kritieke Bedrijfsfuncties
                    - generic [ref=e347] [cursor=pointer]: Dialogue
                    - generic [ref=e350] [cursor=pointer]: Chat
                    - generic [ref=e353] [cursor=pointer]: Information systems
                    - generic [ref=e356] [cursor=pointer]: Related
                    - generic [ref=e359] [cursor=pointer]: Tasks
                    - generic [ref=e362] [cursor=pointer]: Key Indicators
                    - generic [ref=e365] [cursor=pointer]: History
                    - generic [ref=e368] [cursor=pointer]: mo data
                    - generic [ref=e371] [cursor=pointer]: Risks
              - tabpanel [ref=e373]:
                - generic [ref=e381]:
                  - button [ref=e383] [cursor=pointer]:
                    - generic [ref=e384]:
                      - img [ref=e386]
                      - generic [ref=e388]: Test_1780566166653
                      - button [ref=e390]: more_vert
                    - img [ref=e392]
                    - generic [ref=e394]:
                      - button [ref=e395]:
                        - img
                      - checkbox [checked] [ref=e396]
                      - button [ref=e399]:
                        - img
                  - button [ref=e401] [cursor=pointer]:
                    - generic [ref=e402]:
                      - img [ref=e404]
                      - generic [ref=e406]: Questionnaire_1780566166653
                      - button [ref=e408]: more_vert
                    - img [ref=e410]
                    - generic [ref=e412]:
                      - button [ref=e413]:
                        - img
                      - checkbox [checked] [ref=e414]
                      - button [ref=e417]:
                        - img
        - button [ref=e418] [cursor=pointer]:
          - generic [ref=e419]: add
  - dialog "Variables add ..." [ref=e422]:
    - generic [ref=e423]:
      - generic [ref=e425]:
        - heading "Variables" [level=2] [ref=e427]
        - button "add" [ref=e428] [cursor=pointer]:
          - generic [ref=e429]: add
      - generic [ref=e430]:
        - checkbox "Share variables with items also linked in the tab of" [ref=e433] [cursor=pointer]:
          - generic [ref=e436]: Share variables with items also linked in the tab of
        - generic [ref=e437]:
          - generic [ref=e438]:
            - generic: VARIABLE
            - textbox "VARIABLE" [ref=e440]: "100"
          - button "delete" [ref=e444] [cursor=pointer]:
            - generic [ref=e445]: delete
        - generic [ref=e446]:
          - generic [ref=e447]:
            - generic: PERCENTAGE
            - textbox "PERCENTAGE" [ref=e449]: "200"
          - button "delete" [ref=e453] [cursor=pointer]:
            - generic [ref=e454]: delete
      - button "CLOSE" [active] [ref=e456] [cursor=pointer]
```

# Test source

```ts
  1   | import { Locator, Page, expect } from '@playwright/test';
  2   | import fs from 'fs';
  3   | import dayjs from 'dayjs';
  4   | 
  5   | export class UIActions {
  6   |   constructor(private page: Page) { }
  7   | 
  8   |   async click(element: Locator) {
  9   |     await expect(element).toBeVisible();
  10  |     await expect(element).toBeEnabled();
> 11  |     await element.click();
      |                   ^ TimeoutError: locator.click: Timeout 15000ms exceeded.
  12  |   }
  13  | 
  14  |   async doubleClick(element: Locator) {
  15  |     await expect(element).toBeVisible();
  16  |     await element.dblclick();
  17  |   }
  18  | 
  19  |   async rightClick(element: Locator) {
  20  |     await expect(element).toBeVisible();
  21  |     await element.click({ button: 'right' });
  22  |   }
  23  | 
  24  |   async hover(element: Locator) {
  25  |     await expect(element).toBeVisible();
  26  |     await element.hover();
  27  |   }
  28  | 
  29  |   async type(element: Locator, value: string) {
  30  |     await expect(element).toBeVisible();
  31  |     await expect(element).toBeEnabled();
  32  |     await element.clear();
  33  |     await element.fill(value);
  34  |     await expect(element).toHaveValue(value);
  35  |   }
  36  | 
  37  |   async typeWithoutClearing(element: Locator, value: string) {
  38  |     await expect(element).toBeEnabled();
  39  |     await element.type(value);
  40  | 
  41  |     const currentValue = await element.inputValue();
  42  |     expect(currentValue).toContain(value);
  43  |   }
  44  | 
  45  |   async clear(element: Locator) {
  46  |     await expect(element).toBeEnabled();
  47  |     await element.clear();
  48  |     await expect(element).toHaveValue('');
  49  |   }
  50  | 
  51  |   async check(element: Locator) {
  52  |     // await expect(element).toBeVisible();
  53  |     await element.check();
  54  |     await expect(element).toBeChecked();
  55  |   }
  56  | 
  57  |   async uncheck(element: Locator) {
  58  |     await expect(element).toBeVisible();
  59  |     await element.uncheck();
  60  |     await expect(element).not.toBeChecked();
  61  |   }
  62  | 
  63  |   async selectByValue(element: Locator, value: string) {
  64  |     await element.selectOption(value);
  65  |     await expect(element).toHaveValue(value);
  66  |   }
  67  | 
  68  |   async selectByLabel(element: Locator, label: string) {
  69  |     await element.selectOption({ label });
  70  |   }
  71  | 
  72  |   async getText(element: Locator): Promise<string> {
  73  |     await expect(element).toBeVisible();
  74  |     return (await element.textContent()) ?? '';
  75  |   }
  76  | 
  77  |   async verifyText(element: Locator, expected: string) {
  78  |     await expect(element).toHaveText(expected);
  79  |   }
  80  | 
  81  |   async verifyContainsText(element: Locator, partial: string) {
  82  |     await expect(element).toContainText(partial);
  83  |   }
  84  | 
  85  |   async verifyValue(element: Locator, expected: string) {
  86  |     await expect(element).toHaveValue(expected);
  87  |   }
  88  | 
  89  |   async verifyCount(element: Locator, expected: number) {
  90  |     await expect(element).toHaveCount(expected);
  91  |   }
  92  | 
  93  |   async verifyAttribute(element: Locator, attributeName: string, expected: string) {
  94  |     await expect(element).toHaveAttribute(attributeName, expected);
  95  |   }
  96  | 
  97  |   async verifyChecked(element: Locator) {
  98  |     await expect(element).toBeChecked();
  99  |   }
  100 | 
  101 |   async verifyNotChecked(element: Locator) {
  102 |     await expect(element).not.toBeChecked();
  103 |   }
  104 | 
  105 |   async verifyAttached(element: Locator) {
  106 |     await expect(element).toBeAttached();
  107 |   }
  108 | 
  109 |   async verifyVisible(element: Locator) {
  110 |     await expect(element).toBeVisible();
  111 |   }
```